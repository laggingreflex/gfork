...
module.exports = class Config extends Configucius {
  ...
  static options = {
    ...
    debug: { type: 'boolean', alias: ['d'], description: `Log debug messages` },
  ...
  $promptAuth() {
    ...
    const enter = async () => {
      console.log(`Leave blank unneeded fields`);
      await this.$prompt('username');
      await this.$prompt('token');
      await this.$prompt('password');
      await this.$prompt('otp');
      // await Configucius.prompt.select({
      //   message: 'Authenticate using:',
      //   choices: {
      //     token: () => this.$prompt('token'),
      //     password: async () => {
      //       await this.$prompt('password');
      //       await this.$prompt('otp', { message: 'otp (leave blank if not using 2FA)' });
      //     },
      //   }
      // })
      // await this.$prompt('token');
