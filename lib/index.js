...
module.exports = class Gfork extends CLI {

  static path = ['~/.gfork/config.json'];

  log = _.logger();

  gfork = command({
    handler: async function gfork({
      library = this.library,
      skipAuth = this.skipAuth,
      fork = this.fork,
      directory: originalDirectory = this.directory,
      http = this.http,
      domain = this.domain,
      token = this.token,
    } = {}) {
      ...
      if (npmRepoName && this.npmLink !== false) {
