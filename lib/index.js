...
module.exports = class Gfork {
  ...
  async gfork({
    ...
    if (!skipAuth) {
      ...
      if (this.authenticated && fork !== false) {
        forked = await this.fork({ library });
      }
    }
    if (!forked && this.config.username && await this.config.$prompt('assumeFork', {
        message: `Assume fork: '${this.config.username}/${repo}'?`,
        default: true,
      }) !== false) {
      const owner = this.config.username;
      forked = { owner, repo, url: await GitHub.generateUrl({ owner, repo, http, domain, token }) };
    }
    ...
    const npmRepoName = await this.getNpmRepoName({ directory, repo });
    if (npmRepoName !== repo) {
  ...
  async fork({
    ...
    await this.authenticate();
    repo = await this.github.fork({ owner, repo });
    owner = this.github.user;
    const url = await GitHub.generateUrl({ http, domain, token, owner, repo });
  ...
  async getNpmRepoName({ directory, repo }) {
