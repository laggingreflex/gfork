...
module.exports = class Gfork {
  ...
  async pullRequest() {
    const remoteSrc = await this.getRemoteSrc();
    const { current: branch } = await this.git.branch();
    const { owner, repo } = await GitHub.getOwnerRepoFromUrl(remoteSrc);
  ...
  async fetchPr() {
    const remoteSrc = this.getRemoteSrc();
  ...
  async getRemoteSrc() {
    ...
    let remoteSrc;
    while (!(remoteSrc = remotes[this.config.remote])) {
      if (this.config.prompt) {
        await this.config.$prompt('remote', {
          message: `Remote source "${this.config.remote}" not found. Try another?`,
          skipIfExists: false,
        });
      } else {
        throw new _.UserError(`Remote source (${this.config.remote}) not found`);
      }
    }
    return remoteSrc;
  }
